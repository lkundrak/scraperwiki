...
class Migration:
    no_dry_run = True # Data migration
    
    def forwards(self, orm):
        for c in orm.Code.objects.all():
            if c.deleted:
                c.privacy_status = 'deleted'
            elif not c.published:
                c.privacy_status = 'private'
            elif orm.UserCodeRole.objects.filter(code=c, role='email').count() > 0:
                c.privacy_status = 'visible'
            else:
                c.privacy_status = 'public'
            c.save()
